...
<div class="max-w-4xl mx-auto mt-10">
    ...
    <div class="flex justify-end mt-6 space-x-3">
        <a href="{{ route('admin.region.colis.index') }}" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400 transition">⬅ Retour</a>
        ...
        <form action="{{ route('admin.region.colis.destroy', $colis) }}" method="POST" onsubmit="return confirm('Supprimer ce colis ?')">
            @csrf @method('DELETE')
            <button class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition">Supprimer</button>
        </form>
        <a href="{{ route('admin.region.colis.ticket', $colis) }}" target="_blank" class="bg-blue-600 text-white px-4 py-2 rounded mb-4 hover:bg-blue-700 transition">Imprimer le ticket</a>
    </div>
</div>
@endsection
