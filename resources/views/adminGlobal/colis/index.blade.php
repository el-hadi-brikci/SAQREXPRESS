...
@section('content')
<h1 class="text-3xl font-bold text-saqr-blue mb-6">📦 Liste des Colis</h1>
...
<div class="flex flex-col md:flex-row md:justify-between mb-4 space-y-2 md:space-y-0 md:space-x-4">
    <form method="GET" action="{{ route('admin.global.colis.index') }}" class="flex space-x-2">
    ...
    <form method="GET" action="{{ route('admin.global.colis.index') }}" class="flex space-x-2">
        ...
        @if(request('jour'))
            <a href="{{ route('admin.global.colis.index') }}" class="ml-2 text-saqr-blue underline">Réinitialiser</a>
        @endif
    </form>

    <a href="{{ route('admin.global.colis.create') }}" 
...
<div class="overflow-x-auto">
    <table class="min-w-full bg-white border border-gray-200 shadow">
        ...
        <tbody>
            @php
                // Regrouper les colis par date (d/m/Y)
            ...
            @endphp
            @foreach($dates as $dateColis)
                @php
                    $jourColis = $grouped[$dateColis];
                @endphp
                <tr class="bg-gray-100"><td colspan="7" class="font-bold text-saqr-blue py-2 px-3">Journée du {{ $dateColis }}</td></tr>
                @foreach($jourColis as $i => $coli)
                    <tr class="border-t hover:bg-gray-50">
                        ...
                        <td class="py-2 px-3 flex space-x-3">
                            <a href="{{ route('admin.global.colis.show', $coli) }}" class="text-blue-600 hover:underline">Voir</a>
                            <a href="{{ route('admin.global.colis.edit', $coli) }}" class="text-yellow-600 hover:underline">Modifier</a>
                            <form action="{{ route('admin.global.colis.destroy', $coli) }}" method="POST" onsubmit="return confirm('Supprimer ce colis ?')">
                                @csrf @method('DELETE')
